(() => {
  ...
  function stats(){
    const text=flat(),n=countWords(text),active=candidates.filter(c=>!c.reverted),tw=active.reduce((sum,c)=>sum+Math.max(1,countWords(c.original)),0);
    words.textContent=n.toLocaleString();changes.textContent=active.length.toLocaleString();touched.textContent=n?`${Math.min(100,Math.round(tw/n*100))}%`:'0%';if(snapshot&&!manualMode)changeCount.textContent=active.length?`${active.length} suggested edit${active.length===1?'':'s'}`:'No active edits';
    updateDepth(text);
  }
  function updateDepth(text){
    ...
    if(!baseline||!text){
      depth.textContent='0';depthLabel.textContent='Rewrite depth';targetState.textContent='No working copy';
      calcOriginalTokens.textContent='0';calcRevisedTokens.textContent='0';calcTurnover.textContent='0%';calc3.textContent='0%';calc5.textContent='0%';calcCoverage.textContent='0/10';calcStructures.textContent='0';calcMethod.textContent='—';calcFormula.textContent='—';return;
    }
  ...
  function leaveManual(commit){
    ...
    if(commit){snapshot=manual.value;candidates=[]}
  ...
  source.addEventListener('input',()=>{sourceStats();if(baseline&&source.value!==baseline){baseline='';snapshot='';candidates=[];manualMode=false;output.style.display='block';manual.style.display='none';output.className='output empty';output.textContent='Text changed. Offer edits when you are ready.';enable(false);stats();changeCount.textContent='No edits yet'}});
  ...
  sample.addEventListener('click',()=>{source.value=SAMPLE;baseline='';snapshot='';candidates=[];enable(false);sourceStats();stats();source.focus();say('Example loaded. Tap Offer edits.')});
  clear.addEventListener('click',()=>{source.value='';baseline='';snapshot='';candidates=[];manualMode=false;output.style.display='block';manual.style.display='none';output.className='output empty';output.textContent='Your working copy will appear here.';enable(false);sourceStats();stats();changeCount.textContent='No edits yet';source.focus()});
